...
<script type="text/javascript">
    ...
    function queryPage(page, pagesize) {
        ...
        $.ajax({
            type: "post",
            data: jsonObj,
            url: "${APP_PATH}/process/processlists.do",
            beforeSend: function () {
            ...
            success: function (result) {
                ...
                if (result.success) {
                    ...
                    var processs = result.pageInfo.list;
                    var content = "";



                    //i是从0开始的
                    $.each(processs, function (i, process) {
                        content += '<tr>';
                        content += '    <td> + (i + 1) + '</td>';
                        ...
                        content += '    <td>';
                        content += '    <button type="button" onclick="window.location=\'${APP_PATH}/process/toUpdate.htm?id=' + process.id + '\'" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-eye-open"></i></button>';
                        content += '    <button type="button" onclick="deleteProcess(' + process.id + ')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
                        content += '    </td>';
                        content += '</tr>';
                    });
                    $("tbody").html(content);
    ...
    function deleteProcess(id) {
        layer.confirm("确认要删除用户吗?", {icon: 3, title: '提示'}, function (cindex) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.success) {
                        window.location = "${APP_PATH}/process/index.htm"
                    } else {
