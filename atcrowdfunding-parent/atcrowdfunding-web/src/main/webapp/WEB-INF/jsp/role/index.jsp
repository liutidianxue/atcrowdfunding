	...
	<script type="text/javascript">
			...
			function queryPage(page, pagesize) {
				...
				$.ajax({
					type: "post",
					data: jsonObj,
					url: "${APP_PATH}/role/rolelists.do",
					beforeSend: function () {
					...
					success: function (result) {
						...
						if (result.success) {
							...
							var roles = result.pageInfo.list;
							var content = "";
							...
							$.each(roles, function (i, role) {
								...
								content += '	<td>';
								content += '		<button type="button" onclick="window.location=\'${APP_PATH}/role/assignPermission.htm?id=' + role.id + '\'" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
								content += '		<button type="button" onclick="window.location=\'${APP_PATH}/role/toUpdate.htm?id=' + role.id + '\'" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
			...
			function deleteRole(id) {
				layer.confirm("确认要删除用户吗?", {icon: 3, title: '提示'}, function (cindex) {
					...
					$.ajax({
						...
						success: function (result) {
							if (result.success) {
								window.location = "${APP_PATH}/role/index.htm"
							} else {
