          ...
          <div class="table-responsive">
            <table class="table  table-bordered">
              <thead>
                <tr >
                  <th width="30">#</th>
				  <th width="30"><input type="checkbox" id="allSelected"></th>
                  <th>名称</th>
                  <th width="100">操作</th>
                </tr>
              </thead>
        ...
        <script type="text/javascript">
            ...
            function queryPage(page, pagesize) {
                ...
                $.ajax({
                    type: "post",
                    data: jsonObj,
                    url: "${APP_PATH}/cert/certlists.do",
                    beforeSend: function () {
                    ...
                    success: function (result) {
                        ...
                        if (result.success) {
                            ...
                            var certs = result.pageInfo.list;
                            var content = "";
                            ...
                            $.each(certs, function (i, cert) {
                                content += '<tr>';
                                content += '	<td>' + (i + 1) + '</td>';
                                content += '	<td><input type="checkbox" id="' + cert.id +'" ></td>';
                                content += '	<td>' + cert.name + '</td>';
                                content += '	<td>';
                                content += '		<button type="button" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
                                content += '		<button type="button" onclick="window.location=\'${APP_PATH}/cert/toUpdate.htm?id=' + cert.id + '\'" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
                                content += '		<button type="button" onclick="deleteCert(' + cert.id + ')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
                                content += '	</td>';
                                content += '</tr>';
                            });
                            $("tbody").html(content);
            ...
            function deleteCert(id) {
                layer.confirm("确认要删除用户吗?", {icon: 3, title: '提示'}, function (cindex) {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.success) {
                                window.location = "${APP_PATH}/cert/index.htm"
                            } else {
